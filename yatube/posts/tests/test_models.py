...
class PostModelTest(TestCase):
    ...
    def test_models_have_correct_object_names_post(self):
        """Проверяем, что у моделей постов корректно работает __str__."""
        post = PostModelTest.post
        expected_object_name_post = post.text[:CHAR_LIMIT]
        self.assertEqual(expected_object_name_post, str(post))

    def test_models_have_correct_object_names_group(self):
        """Проверяем, что у моделей групп корректно работает __str__."""
